...
| `docs/verification_artifacts/r11b_skew_edge_impact_2026-06-27/r11b_skew_edge_driver.py` | R11b skew-edge dollar-impact A/B driver (PR #437 validation) — adapted from the D23 r11_dollar_impact driver. Two arms over one shared daily rank: `suppressed` (R11a only = pre-#437 live engine) vs `active` (R11a + R11b). Both apply R11a; the only difference is R11b (`vix>25 AND premium_source=="market_mid" AND edge_vs_fair>0`). Requires the real-premium rail (`SWE_OPTION_PREMIUM_DIR`). Emits per-VIX-bucket Δ NAV/Sharpe + blocked-set counterfactual + §2 scan. `--analyze` re-emits markdown. Observation-only; §2-safe. |
| `docs/verification_artifacts/r11b_skew_edge_impact_2026-06-27/FINDINGS.md` | R11b A/B findings. **Verdict: §2-safe but net-costly over a full cycle — do not merge #437 as-is.** W3 (2020-24) −14.85pp / W4 (2021-25) −12.71pp NAV; the blocked trades are net winners in both windows (skew/VRP premium is earned compensation, positive across all VIX buckets). R11b's blunt VIX-level trigger fires post-spike and forgoes recovery premium; only the acute-crash pilot showed it averting losses. Recommends close, or persistence-gated redesign + re-validate. |
| `docs/verification_artifacts/r11b_skew_edge_impact_2026-06-27/w3_2020_2024_100t/summary.json` | W3 (2020-2024, 100t, full friction) machine-readable A/B summary — per-arm NAV/Sharpe/opens/assignments, R11b-blocked counterfactual, per-VIX-bucket, §2 scan, setup fingerprint. |
| `docs/verification_artifacts/r11b_skew_edge_impact_2026-06-27/w4_2021_2025_100t/summary.json` | W4 (2021-2025, 100t, full friction) machine-readable A/B summary (same schema as W3). |
| `docs/verification_artifacts/real_premium_rail_audit_2026-06-27/audit_rail.py` | Real-premium rail data-quality + wiring audit (companion to the R11b work; audits the gitignored `data_processed/option_premium` rail from PR #435). Six checks: coverage %, split-adjust scale, edge_vs_fair units, edge-by-VIX-regime, quote sanity, PIT/no-lookahead. Light (rank calls + parquet scans). Requires `SWE_OPTION_PREMIUM_DIR`. |
| `docs/verification_artifacts/real_premium_rail_audit_2026-06-27/FINDINGS.md` | Rail audit findings. **Verdict: rail wired accurately, no defects** — coverage ~25% market_mid, split-adjust correct (incl. NVDA 10:1), edge_vs_fair positive across all VIX regimes, quotes clean, PIT/no-lookahead clean (4/4). One cosmetic finding: `edge_vs_fair` reported per-contract while `premium`/`fair_value` are per-share. |
