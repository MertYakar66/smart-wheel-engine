...
| `docs/verification_artifacts/f4_post_fix_2026-05-27_raw_output.txt` | Post-fix companion to `f4_baseline_2026-05-26_raw_output.txt`. Captures the same three F4 cases after Fix B1+C ships on `claude/fix-f4-regime-conditioned-widening` @ ec3d51b. Headline: UNH gap CLOSED (ev$ +$114 → −$118, cvar_5 past −10% of collateral, prob_profit 0.86 → 0.77); COST partial preserved by worst-of-two (ev$ +$62 → −$25, prob_profit 0.833 unchanged); AAPL control flagged on legitimate HMM crisis signal. Every value is real Bloomberg-data-driven engine output, not hardcoded. |
