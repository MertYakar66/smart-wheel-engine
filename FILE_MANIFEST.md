...
| `backtests/regression/mag7_reliability.py` | MAG7-only reliability study driver (2026-06-14): eight regime-diverse one-year wheel campaigns at $200k on AAPL/MSFT/GOOGL/AMZN/META/NVDA/TSLA via `run_backtest_multi_friction` (caps-off canonical, 3 friction levels). Mirrors `docs/MAG7_RELIABILITY_2026-06.md`; companion analyzer `scripts/mag7_analysis.py`. |
...
| `docs/MAG7_RELIABILITY_2026-06.md` | MAG7 wheel reliability study (2026-06-14): eight $200k one-year windows on the Magnificent Seven. 7/8 positive (mean +7.6%, worst -17.1% in 2022, which beat Mag7 B&H by +30pp); wheel lags B&H in every bull window (-44..-92pp); EV-selection edge positive+significant on the traded ev>0 region (rho +0.19..+0.66) while the full-sample rho is a non-traded-conservatism artifact; prob_profit top-bin over-confident -20pp; extreme concentration (peak single-name 18-35% NAV, undeployable under R9/R10); verified no-cheat settlement 107/108 + determinism PASS. Driver `backtests/regression/mag7_reliability.py`. |
...
| `scripts/mag7_analysis.py` | Post-processor for the MAG7 reliability campaign: per-window returns/drawdown/funnel, pooled prob_profit + EV-decile calibration, single-name concentration (reconstructed from open+closed positions, self-checked against the tracker's `num_positions`), and an equal-weight Mag7 buy-and-hold benchmark. Reads `%TEMP%/mag7_backtest/` artifacts + the connector. |
