...
| `docs/DATA_FIX_2026-06-28_OHLCV_SPLIT_SCALE_439.md` | Mac-terminal data-fix record for the 2026-03-23 OHLCV split-scale splice (#439 / D-W1-1; data lane, no decision trio). Documents the defect (BKNG 25:1 / CVNA 5:1 adjusted only on the recent slice → fake −96 %/−80 % seam return), the two-method completeness proof that the split-affected set is exactly {BKNG, CVNA}, the full-history back-adjustment convention + lockstep BKNG dividend ÷25, connector-served verification (0 remaining breaks, frontier unchanged), the three flipped test pins, the **new** FDX/APTV spinoff-splice finding (separate defect), and the cross-terminal handoff to re-baseline the s27/s32/s34/s35 regression snapshots (#402/Windows lane). Reproduced by `scripts/fix_ohlcv_split_scale_439.py`. |
...
| `docs/DATA_FIX_2026-06-28_OHLCV_SPLIT_SCALE_439.md` | Fix report for the BKNG 25:1 / CVNA 5:1 split-scale splice (#439/#455): defect, root cause, repair method, verification; folded into the 2026-07-02 frontier bump (#472). |
...
| `docs/verification_artifacts/real_premium_rail_audit_2026-06-27/audit_rail.py` | Real-premium rail data-quality + wiring audit — validates the merged #435 real-premium wiring (the gitignored `data_processed/option_premium` rail, local-only). Six read-only checks: coverage % (market_mid vs synthetic fallback), split-adjust scale correctness, edge_vs_fair units, edge-by-VIX-regime, quote sanity, PIT/no-lookahead. Requires `SWE_OPTION_PREMIUM_DIR`. Complements the Mac's rail-INDEPENDENT campaign (#436). |
| `docs/verification_artifacts/real_premium_rail_audit_2026-06-27/FINDINGS.md` | Rail audit findings. **Verdict: the real-premium data is wired accurately — no defects.** Coverage ~25% market_mid; split-adjust correct (incl. NVDA 10:1); edge_vs_fair positive across all VIX regimes (the VRP/skew premium is real); quotes clean; PIT/no-lookahead clean (4/4 — so no backtest is future-contaminated). One cosmetic note: `edge_vs_fair` is reported per-contract while `premium`/`fair_value` are per-share. |
| `docs/verification_artifacts/real_premium_rail_audit_2026-06-27/out/summary.json` | Machine-readable rail-audit results (C1-C6) for `FINDINGS.md`. |
| `docs/verification_artifacts/skew_edge_selection_2026-06-28/skew_edge_selection.py` | Forward-research driver: is the real skew-edge (`edge_vs_fair`) a usable SELECTION signal? Weekly 2020-2024 100t cross-sectional study — sorts `market_mid` candidates by edge / edge-richness into quintiles, forward-replays short-put realized return (normalized by collateral), stratifies by VIX regime, reports Spearman. Read-only; requires `SWE_OPTION_PREMIUM_DIR`. |
| `docs/verification_artifacts/skew_edge_selection_2026-06-28/FINDINGS.md` | Skew-edge selection findings. **Null result: NOT a clean lever.** The overall positive Spearman (+0.28-0.31) is a REGIME CONFOUND (high-edge clusters in crisis, where put-selling pays +1.55%; low-edge in calm, −0.93%); within-regime the lever is weak/inconsistent (anti-monotone in calm). Recommendation: document, build nothing — the edge's value is in ranking/calibration (#435 wiring), not as a cross-sectional selector. |
| `docs/verification_artifacts/skew_edge_selection_2026-06-28/out/summary.json` | Machine-readable skew-edge study results (overall + by-edge/richness quintile + by-VIX-regime). |
| `docs/verification_artifacts/s402_drift_attribution_2026-06-28/diag_s402_iv_gate_footprint.py` | #402 root-cause attribution diagnostic (read-only): scans the #363 IV-sentinel gate (`(3.0, 10000]` band) footprint over the exact s27/s32/s34 reproducer windows (UNIVERSE_24 2022-2024, UNIVERSE_100 2024), normalizing tickers as the connector does on load. Quantifies how many in-window `hist_*_imp_vol` cells the gate NULLs → emits `iv_gate_footprint.json`. |
| `docs/verification_artifacts/s402_drift_attribution_2026-06-28/iv_gate_footprint.json` | Machine-readable #363-gate footprint over the s27/s32/s34 windows: per-window non-null IV-cell counts and gated (out-of-band) counts. Result: **0 gated cells** in every window — the #363 prime suspect cannot move those reproducers' metrics via the PIT path. |
| `docs/verification_artifacts/s402_drift_attribution_2026-06-28/check_snapshot_fingerprints.py` | #402 decisive check (read-only): compares each committed s27/s32/s34 snapshot `fingerprint` (ohlcv/vol_iv/treasury + the 10-file connector_data_sha256) to the live data hashes. All-MATCH + deterministic engine + last EV-moving merge = #433 ⇒ reproducers produce committed metrics by construction (exact identity, not within-tolerance). Faster than the multi-hour backtest. Emits `fingerprint_check.json`. |
| `docs/verification_artifacts/s402_drift_attribution_2026-06-28/fingerprint_check.json` | Machine-readable fingerprint-identity result: per-snapshot MATCH/MISMATCH for each pinned input. Result: **ALL-MATCH** across all three reproducers → no live drift → #402 stale. |
| `docs/verification_artifacts/s402_drift_attribution_2026-06-28/FINDINGS.md` | #402 snapshot-drift attribution. **Verdict: #402 is STALE** — the #338-baseline drift was absorbed by two later re-baselines (#429 PIT carry-q, #433 corporate-action lockout), the #363 prime suspect is refuted (zero in-window footprint), and the fingerprint blind-spot it cited (unpinned IV/treasury) is closed (the fingerprint now pins the full connector data set). Recommends closing #402; no new re-baseline warranted. |
...
| `docs/HEAVY_VERIFY_2026-06-29_DATA_WIRING_RELIABILITY.md` | Mac-terminal independent re-verification of the merged #436 W1–W2 (data-wiring accuracy + engine output realism) on current `main`. Report-only reproduction from drivers written against the connector API (not a reuse of the 2026-06-27 drivers): W1 + W2 reproduce byte-exactly (986 candidates, 0 non-finite/Greek/out-of-band; implied-IV band 2,062,702 cells / 0 violations; 6 split-scale suspects); the 2026-03-23 split-scale splice (D-W1-1) is re-confirmed present (fix held in PR #455) with the corp-action cross-check tightening CVNA to the authoritative 5.0; two minor net-new observations (BIIB/TPL NaN-halt rows; realized-vol correctly not IV-gated). No trio touch. Reproduced by `scripts/audit_w1_data_wiring_reverify.py` + `scripts/audit_w2_output_realism_reverify.py` → `docs/verification_artifacts/data_wiring_reverify_2026-06-29/`. |
| `docs/verification_artifacts/data_wiring_reverify_2026-06-29/*` | Machine-readable per-check JSON for the 2026-06-29 independent #436 W1–W2 re-verification: `w1_ohlcv.json` (universe integrity + split-scale sweep), `w1_corp_action_crosscheck.json` (BKNG 25.0 / CVNA 5.0 authoritative split ratios + effective dates), `w1_nflx.json`, `w1_vol_iv.json` (implied-vs-realized gate scoping), `w1_treasury.json`, `w1_generic_sources.json`, `w1_summary.json`, and `w2_<regime>.json` + `w2_summary.json` (per-regime realism). Persisted before pretty-print so a console crash never loses compute. Re-emit with the two `audit_*_reverify.py` drivers. |
| `docs/HEAVY_VERIFY_2026-06-28_TOPBIN_NETCOST.md` | Mac-terminal heavy-verify findings doc (#446, W6): is the calm/elevated-ENTRY top-bin (`prob_profit`>0.90) over-confidence (W3/#442) net-dollar-COSTLY, or still net-positive premium (the R11b lesson)? Validation/measurement-only (no engine-behaviour edits). Records the forward-replay method, per-window + pooled top-bin vs baseline economics at full friction (cluster-bootstrap CIs), the realized-vs-forecast calibration gap (reproducing W3), and the net-cost verdict — with either a one-paragraph rule spec for the Windows terminal (if net-costly) or a 'no rule, document-only' conclusion invoking the R11b precedent (#437). Reproduced by `scripts/audit_topbin_netcost.py` → `docs/verification_artifacts/topbin_netcost_2026-06-28/`. |
| `docs/verification_artifacts/topbin_netcost_2026-06-28/*` | Machine-readable artifact for the #446 W6 top-bin net-cost measurement: `w6_topbin_netcost.json` (per-window + pooled bucket economics — n, mean/total realized $/contract at none/bid_ask/full friction, cluster-bootstrap + Wilson CIs, calibration gap, ITM rate — plus the programmatic verdict). Persisted before pretty-print so a console crash never loses the ~40-min full-universe compute. Re-emit with `python scripts/audit_topbin_netcost.py`. |
| `docs/HEAVY_VERIFY_2026-06-28_FULL_WHEEL_REALISM.md` | Mac-terminal heavy-verify findings doc (#450, W7 capstone): does full-wheel (put→assignment→covered-call→recovery) realized P&L stay net-positive and does the engine's EV ranking survive the leg it never directly scored? Validation/measurement-only (no engine-behaviour edits; engine selects every put + covered call). Records the per-cycle method, full-wheel realized $/cycle vs put-leg-only vs capital-matched buy-and-hold vs ex-ante EV (by VIX-at-entry / named windows / disjoint halves), assignment economics, the Spearman(EV, full-cycle) ranking-authority test, and the end-to-end verdict — closing the put-leg-only caveat across the campaign. Reproduced by `scripts/audit_full_wheel.py` → `docs/verification_artifacts/full_wheel_2026-06-28/`. |
| `docs/verification_artifacts/full_wheel_2026-06-28/*` | Machine-readable artifact for the #450 W7 full-wheel capstone: `w7_full_wheel.json` (per-bucket full-cycle economics — n, mean/total full-cycle realized $/cycle incl. and excl. open marks, cluster-bootstrap + Wilson CIs, put-leg/B&H/EV comparisons, assignment + open-mark rates, covered-call cycles to resolution, and the Spearman(EV, full-cycle) ranking test — plus the programmatic verdict). Persisted before pretty-print. Re-emit with `python scripts/audit_full_wheel.py`. |
| `docs/HEAVY_VERIFY_2026-06-29_R11_REFINEMENT.md` | Windows-terminal heavy-verify findings doc (#442 → the `r11-onset-aware-trigger` research card): should R11 become entry-VIX-band-aware or persistence-aware? Measurement-only — R11 (`candidate_dossier.py`) untouched. Verdict: **keep R11 as-is.** A static VIX-band carve-out fails (the "≥30 well-calibrated" basis was a single date; the firing zone is net-costly; the CV threshold θ≥27.5 already fails the 2022 fold), and the persistence-N condition is **refuted** — all whole-book Δ are within noise (paired daily-return |t|≤0.5, both windows) and a strictly-consecutive N-day condition retains only 63-65% of the 2022 averted loss because 2022's VIX>25 was choppy vs 2020 sustained. Reproduced by the drivers in `docs/verification_artifacts/r11_refinement_2026-06-29/`. |
| `docs/verification_artifacts/r11_refinement_2026-06-29/*` | Machine-readable artifacts for the #442 / R11-refinement study: `r11_persist_driver.py` (5-arm persistence A/B extending the shipped `r11_dollar_impact` harness — suppressed / active / persist{5,10,20}, one shared daily rank, gate replicated from the engine's own `R11_*` constants), `r11_band_split.py` (#442 dense-VIX≥25 band-split net-cost via `audit_topbin_netcost.collect`), `w{3,4}_summary.json` + `w{3,4}_daily_nav.csv` (per-arm NAV/Sharpe + blocked-by-regime + daily NAV for the paired t-stats), `band_split_442.json`. §2-safe (the replicated gates only remove opens; no `engine/` file modified). |
...
| `scripts/audit_full_wheel.py` | W7 full-wheel realized-P&L capstone (heavy-verify 2026-06-28, #450, Mac terminal; discovery, read-only, additive — closes the put-leg-only caveat of W3/W5/W6). Per-cycle accountant: the engine selects every short put (`rank_candidates_by_ev`) AND every covered call (`rank_covered_calls_by_ev`) → `EVEngine.evaluate`; this driver only accounts. Simulates put→assignment(basis=strike)→engine-ranked covered-call→called-away/recovery, marking residual shares to market (open mark, reported), at full friction via the canonical `_common` helpers. Reports full-wheel realized $/cycle vs put-leg-only vs capital-matched buy-and-hold vs ex-ante `ev_dollars`, assignment economics by VIX-at-entry, and Spearman(`ev_dollars`, full_cycle) to test whether EV-ranking survives the leg it never scored. Cites I7 roll economics + the R11 dollar-impact caveat. Cluster-bootstrap/Wilson CIs; cap pre-splice (#439). Parallel collect (`--collect-to`)→merge (`--analyze-from`). No §2 bypass. Persists `w7_full_wheel.json`. |
| `scripts/audit_topbin_netcost.py` | W6 top-bin net-cost measurement (heavy-verify 2026-06-28, #446, Mac terminal; discovery, read-only, additive — follow-up to W3/#442). Over a monthly grid spanning both windows (2020-2024, 2021-2025), ranks the full universe via `rank_candidates_by_ev` and forward-replays each short put's held-to-expiry P&L with the canonical `_forward_replay_realized_pnl` + `_spot_on_or_after` helpers at three friction levels (none/bid_ask/**full**), to answer whether the calm/elevated-ENTRY top-bin (`prob_profit`>0.90) over-confidence is net-dollar-COSTLY or still net-positive premium (the R11b lesson, #437). Reports per-window + pooled top-bin vs non-top-bin vs all-candidate baseline with cluster-bootstrap (over as_of) mean CIs + Wilson win-rate CIs; emits a window-agreeing verdict. Supports parallel collect (`--collect-to`) → merge (`--analyze-from`). No §2 bypass. Persists `w6_topbin_netcost.json`. |
...
| `scripts/audit_w1_data_wiring_reverify.py` | W1 independent data-wiring re-verification (heavy-verify 2026-06-29, #436; report-only, additive). Re-derives the merged W1 per-source pass/fail + the OHLCV split-scale defect from the `MarketDataConnector` public API (does not import the 2026-06-27 driver): universe integrity sweep, >2× split-scale classifier with corp-action cross-check (factor + effective-date + uniform-OHLC CV), NFLX refutation, implied-IV band cleanliness (scoped to implied columns; realized vol reported separately), treasury/dividend/fundamental/credit checks. No §2 bypass; decision trio untouched. Persists JSON to `docs/verification_artifacts/data_wiring_reverify_2026-06-29/` before printing. |
| `scripts/audit_w2_output_realism_reverify.py` | W2 independent output-realism re-verification (heavy-verify 2026-06-29, #436; report-only, additive). Re-runs `WheelRunner.rank_candidates_by_ev` over the full universe at the same 5 regime `as_of` dates and re-checks finiteness, `prob_profit`/`prob_assignment` ∈ [0,1], served-IV band, premium/spot sanity, recomputed Greek-contract units, and regime magnitude scaling. Reproduces the published W2 byte-exactly (986 candidates; 0 non-finite / Greek / out-of-band). No §2 bypass; decision trio untouched. Persists per-regime JSON before printing. |
...
| `scripts/check_coverage_floors.py` | CI guard — per-file coverage floors for the decision trio + engine core (ratchet against decay; the aggregate `--cov-fail-under=80` lets one file shed 20pp unseen — adversarial review 2026-07-01). Reads the Test Suite job's `coverage.json`; floors = measured-on-main − 2pp, baseline CI run 28638294523 @ `4c5a1a4`; missing file fails loud. |
| `scripts/fix_ohlcv_split_scale_439.py` | One-shot repair of the 2026-03-23 OHLCV split-scale splice (#439/#455): back-adjusts BKNG/CVNA pre-splice history onto the split-adjusted scale (Decimal-exact, CRLF-preserving, idempotency-guarded per leg). Re-applied to the 2026-07-02 re-pull (#472; dividends leg self-refused — fresh pull already adjusted). |
...
| `tests/test_w7_full_wheel.py` | W7 full-wheel pins (heavy-verify 2026-06-28, #450). Pins the entry-VIX bands, the canonical put-leg reuse (`_forward_replay_realized_pnl`), the rank-correlation helper, and the end-to-end verdict logic (VALIDATED END-TO-END when all cuts net-positive AND EV-ranking holds; positive-but-weak-ranking; MATERIALLY WORSE; INSUFFICIENT n<30) + two live pins on the assignment→covered-call→recovery accounting (OTM ⟹ full==put-leg; assigned ⟹ recovery legs; forward resolution dates). |
| `tests/test_w6_topbin_netcost.py` | W6 top-bin net-cost pins (heavy-verify 2026-06-28, #446). Pins the entry-VIX bands (calm ≤15 / elevated 15-25 / crisis >25), the friction-overlay realized P&L (reuses the canonical `_forward_replay_realized_pnl`; none ≥ bid_ask ≥ full; assignment-slip only when ITM), the cluster-bootstrap CI, and the window-agreeing net-cost verdict logic (INSUFFICIENT at n<30; NET-COSTLY / NET-POSITIVE / DRAG branches) + a cheap live rank→replay integration pin. |
...
| `tests/test_w1_data_wiring.py` | W1 data-wiring pins (heavy-verify 2026-06-27, #436). Verified-property tests (served IV band ∈ (3.0, 10000], no deep-IV sentinel leak, OHLC invariant, monotonic+positive OHLCV, treasury covers the feasible window) plus a behaviour pin that the BKNG/CVNA 2026-03-23 split-scale discontinuity is repaired (#439: seam ratio now the genuine weekend move, in [0.5, 2.0]) — was a strict-xfail until the back-adjust landed. |
| `tests/test_w1w2_reverify.py` | W1/W2 re-verification pins (heavy-verify 2026-06-29, #436). Two durable properties not covered by the W1/W2 pins above: corp-action split ground-truth (`get_corporate_actions` serves BKNG 25:1 eff 2026-04-06, CVNA 5:1 eff 2026-05-08, NFLX 10:1 eff 2025-11-17 — the D-W1-1 root-cause data, durable after PR #455) with the effective-dates-postdate-the-2026-03-23-splice diagnosis pin, and the IV-validity gate `(3.0, 10000]` scoped to implied vol only (realized-vol columns correctly served below 3% for EA/HOLX, never floored or negative). RED-proven. |
...
| `tests/test_common_realized_pnl.py` | Ground-truth dollar value-asserts for the canonical realized-P&L + friction helpers in `backtests/regression/_common.py` (#456 C; fast, no marker). Pins hand-computed values (not shape/finiteness) for `_forward_replay_realized_pnl` (OTM keep / breakeven / ITM / worst-case), `friction_adjusted_premium` (pct vs 0.05-floor vs max(0,·) branches), `friction_open_cost`, `friction_assignment_cost` (10bp+commission, strike/contract scaling), and a composed full-friction ITM put leg (−735.30) — the W7-double-count (#451) bug class. Each literal verified against the helper and shown to sit far from the nearest plausible formula regression. |
...
| `tests/test_wallclock_staleness.py` | D1-2/D3-2 — wall-clock frontier staleness layers: once-per-connector warn at >7d, structured `attrs["staleness"]` on the three rankers, opt-in `refuse_stale_live` / `SWE_REFUSE_STALE_LIVE` universe-wide refusal (drop-only); dated queries return the `{checked: False}` sentinel; default path fail-open. |
| `tests/test_coverage_floors_script.py` | Pins for the per-file coverage-floor ratchet: pass at/above floor, below-floor failure naming the file, MISSING-file loud failure (rename must update FLOORS — no silent skip), Windows path-key normalization, missing-json exit code, floored-set census. |
...
| `tests/test_real_premium_wiring.py` | Pins the real-premium ranker wiring: `_cumulative_split_factor` math + the connector's split-adjust-on-load (raw larder → engine split-adjusted frame), the `wheel_runner._resolve_real_premium` snap helper (expiry/strike tolerance, graceful `None`), the D1-1 quote/spot coherence guards (exact quote-date==spot-bar-date match, DTE-vs-modeled tolerance, as_of=None re-aimed at the spot bar, no-anchor refusal — unit + e2e pins of the 2026-07-01 frontier-skew defect shapes), and the end-to-end puts ranker (uses `premium_source="market_mid"` with the injected mid when the rail covers a name; byte-identical synthetic fallback when absent). |
