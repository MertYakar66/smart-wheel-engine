...
| `scripts/fix_ohlcv_split_scale_439.py` | One-time data-layer remediation for the 2026-03-23 OHLCV split-scale splice (#439 / D-W1-1; Mac terminal — data lane, no decision trio, no connector behaviour). Back-adjusts the pre-splice history of the only two split-straddle names onto the split-adjusted (frontier) scale: `data/bloomberg/sp500_ohlcv.csv` rows `date < 2026-03-23` for BKNG (÷25) / CVNA (÷5) → prices ÷ factor, volume × factor; and in lockstep `data/bloomberg/sp500_dividends.csv` BKNG `dividend_amount` ÷25 (preserves yield; CVNA pays none). Streaming + CRLF-preserving + `Decimal`-exact (÷5/÷25 terminate) + idempotency-guarded, so every untouched row is byte-identical. Factors are the true corporate-action ratios (not raw boundary ratios). Re-baseline of the s27/s32/s34/s35 regression snapshots is handed to the #402/Windows lane. See `docs/DATA_FIX_2026-06-28_OHLCV_SPLIT_SCALE_439.md`. |
