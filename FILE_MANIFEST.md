...
| `backtests/regression/s43_rolling_multiwindow.py` | S43 rolling-window harness — 4 windows (W1=2018-2022, W2=2019-2023, W3=2020-2024 = S38 re-run, W4=2021-2025) at $1M / 100 tickers / three friction levels. Driver for `docs/ENGINE_BACKTEST_S43_ROLLING_MULTIWINDOW.md` (PR #270). |
| `backtests/regression/s43_scan.py` | S43 scan helper — emits per-day rank_log rows for the 4 rolling windows. Cooperates with `s43_rolling_multiwindow.py` to keep memory footprint bounded. |
| `backtests/regression/s43_analyze.py` | S43 analyzer — Spearman ρ + per-year + per-quartile + Univ-EW baseline diff over the 4 rolling-window outputs. Generates the headline + Δ-vs-#260 tables in the S43 doc. |
| `backtests/regression/s43_reconstruct.py` | S43 W1 reconstruction helper — replays rank_log to derive the running-peak R9/R10 audit values that the original tracker_state was missing for W1. Documents the synthesis in §1 of the S43 doc. |
...
| `docs/ENGINE_BACKTEST_S43_ROLLING_MULTIWINDOW.md` | S43 — Rolling 5-window backtest at $1M / 100 tickers on the post-#260 engine. Four windows (W1=2018-2022, W2=2019-2023, W3=2020-2024, W4=2021-2025). Engine never beats Univ-EW baseline (range −51pp to −104pp). Spearman ρ window-invariant 0.356-0.378. F4 fix (PR #260) signal-preserving on W3 ⟷ S38 comparison. R10 would-fire 3.7-4.5% of executed opens, materially bounding single-name exposure. |
| `docs/ENGINE_REVERIFY_S46_POST_F4_R10.md` | S46 — Re-verification report against `origin/main` @ `56d8e5c` (post-#260 RV30/RV252 widening + post-#262 R10 cap). Headline: engine solid against re-verified surfaces. Realism battery byte-identical pre-vs-post #260; F4 fix reproduces UNH widening (-$6.28) to the cent; HMM regime + composition identity reproduce to documented tolerances. Two expected drifts (S31 V1 credit_mult, S37 crisis-day ρ) footnoted as environment/signal effects. Three sharp findings filed in §5 for follow-on hardening. |
...
| `docs/verification_artifacts/realism_2026-05-28_raw_output.txt` | S46 re-run of `realism_verify_driver.py` against `origin/main` @ `56d8e5c` (post-#260 + post-#262). Byte-identical to the 2026-05-26 pre-#260 baseline across all 5 sections — F4 widening doesn't fire on any of the realism driver's anchor dates (all sit below the 1.30 threshold). Confirms #260+#262 do not regress the realism battery. |
| `docs/verification_artifacts/f4_baseline_2026-05-28_raw_output.txt` | S46 re-run of `f4_baseline_driver.py` on the post-#260 engine. UNH 2024-11-11 widens ev_dollars +$114.53 → +$108.25 and cvar_5 −$2528.13 → −$2608.69 (matches PR #260 + Terminal A's S41 to the cent). COST 2022-04-04 byte-identical (rv30/rv252 below 1.30 threshold — fix correctly silent). AAPL 2026-02-13 control byte-identical. Validates the F4 fix mechanism on its own canonical cases. |
