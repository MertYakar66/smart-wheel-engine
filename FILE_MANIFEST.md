...
| `backtests/regression/sim200k_reliability.py` | SIM-200K harness — eight one-year windows at $200k retail scale (crash-entry 2020 through recent 2025), canonical S38/S43 knobs, three friction levels. Mirrors `docs/SIM_200K_RELIABILITY_2026-06.md`. |
...
| `docs/SIM_200K_RELIABILITY_2026-06.md` | SIM-200K campaign writeup — eight one-year wheel runs at $200k from regime-diverse start dates (crash-entry 2020 → recent 2025), full no-lookahead protocol, honest-limitations register, per-window results / calibration / concentration / benchmark tables, independent adversarial verification. Mirrors `backtests/regression/sim200k_reliability.py`. |
...
| `scripts/sim200k_analysis.py` | SIM-200K post-processor — returns/drawdown from equity curve, funnel, EV-decile + prob_profit reliability tables, friction sensitivity, running-peak R9/R10 would-fire audit, EW-universe benchmark. Reads `%TEMP%` campaign artifacts; read-only vs `engine/`. |
| `scripts/sim200k_tier_probe.py` | SIM-200K forward-distribution tier probe — re-ranks each campaign window at start/mid/end dates and tabulates per-row `distribution_source` (the campaign rank logs don't persist it). Read-only. |
