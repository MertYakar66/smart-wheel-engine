...
| `docs/DATA_ACQUISITION_PLAN_2026-06-14.md` | Authoritative Bloomberg pull plan for the university terminal session (compiled 2026-06-14, verified against `origin/main` @ `83eacdd` via the live connector). Part A verified current-state inventory (treasury + corp-actions DONE, `vol_dvd` stale at 2026-03-20, `sp500_iv_surface.csv` absent, dividends 427/503, BKNG/CVNA 2026-03-23 split seam, NFLX mis-scale fixed); Part B what Theta already covers (don't re-pull); Part C the P0/P1/P2 baseline pull list; Part D code-grounded NET-NEW additions per engine subsystem (file:line hooks + §2 role + usable-today vs needs-code); Part E requires-new-code appendix (E-1..E-20); Part F do-not-pull/infeasible/Theta-routed; Part G terminal execution notes. Companion to `docs/DATA_ACQUISITION_ROADMAP.md`, `docs/BLOOMBERG_PULL_LIST.md`, `docs/DATA_INVENTORY.md`. |
