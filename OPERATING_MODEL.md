...
Four rules on top of the tiers:
...
**Magnet files.** The registries touched by nearly every task (`FILE_MANIFEST.md`, `docs/worklog/INDEX.md`, and the temporal docs `CHANGELOG.md` / `PROJECT_STATE.md` / `DECISIONS.md` / `ROADMAP.md`) are the main merge-contention points. The source protocol named one `FILE_MANIFEST.md` owner per cycle (others list new files in their PR body; the owner reconciles, or a `python scripts/sync_manifest.py --fix` pass closes the gaps at cycle close) — a rule that was expressly interim until per-task worklog fragments landed, which they since have; the allocator may still name an owner when contention warrants it, and reconciles the temporal docs at cycle close. On any conflict in these files, resolve mechanically per §6 (union of rows + regenerate the index) — never `git checkout --theirs` (it once silently reverted a 12-doc archive move).
...
- **Documentation:** every model includes its mathematical specification, input parameter definitions and valid ranges, output specifications and bounds, assumptions and limitations, and a reference to an academic or industry source; every model is documented in `docs/MODEL_CARDS.md`. Model-function docstrings carry a "Mathematical basis" section plus Args (with valid ranges/units) / Returns (with bounds) / Raises / References.
...
- **Input data sanity:** prices non-negative and not stale (the original framework pinned staleness at < 15 min — an intraday bound that predates the EOD-only data reality); volumes non-negative integers; IV in the framework's stated range [0.01, 5.0]; Greeks within theoretical bounds. Feature computations log their source-data timestamp, track their transformation steps, and support audit replay.
- **Missing-data defaults:** a missing price falls back to the last valid price with a staleness flag; a missing IV flags the position for manual review; missing Greeks are recomputed from the model.
