...
| `test_drive_consolidate.py` | The D33 Drive consolidation tool on a fake Drive: census completeness, plan classes, collision-safe destinations, SHA-256 decides and a missing hash never matches, credential files never read (on any path, in any area, or under a sweep source), copy never overwrites (hard-link publish), retries, resumes and reruns, Drive changes during a copy (a census before and after the downloads), links and junctions never a home (Python 3.11 and 3.12), `verify` re-hashes what the plan relies on, deletable only for bytes already home and only if every row naming the object is, outputs never through a link in the root (followed hop by hop) nor over their own inputs, the copy log a new file each run, `sweep`, `SHA256SUMS`, the rclone filter |
