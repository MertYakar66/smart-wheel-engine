...
| `test_drive_consolidate.py` | The D33 Drive consolidation tool on a fake Drive: census completeness, plan classes, collision-safe destinations, SHA-256 decides and a missing hash never matches, credential files never read, copy never overwrites (hard-link publish), retries, resumes and reruns, links and junctions never a home, `verify` re-hashes what the plan relies on, deletable only for bytes already home, the second review's findings, no output written through a link, `sweep`, `SHA256SUMS`, the rclone filter |
