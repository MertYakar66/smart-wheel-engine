...
| dead-code deletion (conservative) | ✅ | removed the unreachable numeric-timestamp branch in the TV webhook freshness check. Other candidates resolved differently: the dead LSM tier was **fixed** (O7) not deleted; the two "zero-collect" suspects resolved differently — `test_transaction_costs` collects 23 tests on main (branch-staleness, not dead; kept), but `test_wheel_cycle` genuinely collects 0 (a 48-line `print()` script, covered by `test_wheel_lifecycle`) and is removed in a follow-up branch; `earnings_drift`/`src/`/`models/` intentionally kept per D2/D14. |
