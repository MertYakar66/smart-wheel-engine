...
>
> _Currency note (2026-07-02): counts in this snapshot predate `corporate_actions` joining `_FILES` — the connector now reads **10** monolith files (plus two `broad_pull/` panels, fingerprint-pinned since #465). Current counts: `docs/DATA_POLICY.md` §5._
>
> _Currency note (2026-07-04): the **"3 refreshable / 6-of-9 no-producer"** headline in the body below is now **stale**. The xbbg-puller salvage landed in-repo producers that write the connector CSVs directly — `scripts/pull_vol_iv.py` → `sp500_vol_iv_full.csv`, `scripts/pull_dividends.py` → `sp500_dividends.csv`, `scripts/pull_vix_term_structure.py` → `vix_term_structure.csv`, `scripts/pull_corporate_actions.py` → `sp500_corporate_actions.csv`, plus `scripts/pull_snapshots.py` for snapshot fields (credit-risk / fundamentals). Only `sp500_earnings.csv` (the BDS earnings backfill) remains deferred. **The producer-census tables in this snapshot's body — and the matching "7 have no runnable producer" block in `docs/DATA_POLICY.md` §5 (≈ lines 169-189) — both predate this salvage and are pending operator reconciliation.**_
