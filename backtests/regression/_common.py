...
The driver (:func:`run_backtest`) is the **single** entry point — all
four reproducers route through it. It loops trading days, calls
:meth:`engine.wheel_runner.WheelRunner.rank_candidates_by_ev`
end-to-end (no engine mocking, no §2 bypass), walks the ranked rows
through a :class:`engine.wheel_tracker.WheelTracker`, and computes
the metrics dict that maps 1:1 to the JSON snapshot schema.
