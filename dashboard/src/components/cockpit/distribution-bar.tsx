...
export function DistributionBar({
  ...
  const title =
    `5% tail (CVaR): ${fmt(cvar5)}  ·  ` +
    `P25 ${fmt(p25)} · P50 ${fmt(p50)} · P75 ${fmt(p75)}  ·  ` +
    `breakeven at 0. Body = likely outcome; red whisker = modeled crash tail.`;
  ...
  return (
    ...
    >
      ...
      {/* tail whisker: cvar5 -> box left edge */}
      {cvarPct !== null && whiskerRight > cvarPct && (
        <div
          className="absolute top-1/2 h-[2px] -translate-y-1/2 bg-terminal-red/70"
      ...
      {cvarPct !== null && (
        <div
          className="absolute top-1/2 h-2.5 w-[2px] -translate-y-1/2 bg-terminal-red"
          style={{ left: `${cvarPct}%` }}
        />
      )}
