...
> **Merge status (2026-06-17):** this document and the two validation reports
> are merged to `main` as the record of findings. The **code fixes below are NOT
> yet on `main`** — they were authored against an earlier base, and `main` has
> since advanced ~110 commits (notably `engine/wheel_runner.py` rewritten by
> +530 lines), so the engine edits (D-2, dealer-clamp, HMM flag, provenance,
> coverage gate, tests) require careful re-application + re-validation against
> current `main` as a follow-up PR. R11 was independently propagated to
> `CLAUDE.md` on `main` already. Treat the "FIXED" rows below as "fix authored &
> locally validated, pending re-base," not "live on main."
