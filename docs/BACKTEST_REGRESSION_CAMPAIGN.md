...
|---|---|---|---|
| H1: Driver vs throwaway-harness strike divergence — ~5–20 % NAV / per-quartile-P&L gaps on S27 (−16 %) and S32 (+5.8 %). S34 averages it out (+0.7 %). | open | Terminal A (s39 investigation: `claude/usage-test-s39-s27-trade-count-isolation`) | Cause unconfirmed: likely strike-rounding + CC-entry strike choice that doesn't exactly mirror the lost throwaway harness pattern. |
