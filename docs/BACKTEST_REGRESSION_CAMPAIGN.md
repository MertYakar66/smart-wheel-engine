...
- **Driver vs throwaway-harness strike divergence.** The S22/S27/S32
  throwaway harnesses are lost; my driver's strike selection differs
  marginally. This shows up as ~5–20 % NAV / per-quartile-P&L gaps
  on S27 and S32. S34 averages it out.
- **CSV hydration in CI.** The dedicated `Backtest Regression`
  workflow exists but is dispatch-only. Bloomberg CSV must reach the
  CI runner before the cron lane is enabled.
- **In-sample HMM / POT-GPD parameters.** S35's caveat holds: the
  engine's regime thresholds, dealer multiplier clamp, dropout
  thresholds, and POT-GPD tail calibration were tuned with full
  2018–2026 visibility. None of the four backtests are
  parameter-OOS. A true OOS validation would need a parameter freeze
  + replay setup, which is out of scope.
- **`pre_post_pit_diff` audit not yet wired.** A future enhancement
  could run the same reproducer against a pre-fix engine SHA (via
  `git worktree`) and emit a structured diff — useful when other
  IV-PIT-class bugs surface.
