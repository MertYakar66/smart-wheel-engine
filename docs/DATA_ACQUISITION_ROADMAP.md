...
- **The IV file is ATM-only.** `sp500_vol_iv_full.csv` carries ATM approximations (`hist_put_imp_vol`, `hist_call_imp_vol`), and on Bloomberg `put_iv == call_iv` for ~100% of rows — there is **no skew** in the IV file. True moneyness-by-strike IV (the dormant skew surface) comes from the `xxDAY_IMPVOL_yy.y%MNY_DF` field family via `BDH`, or from Theta per-strike chains. ATM term structure is robust; smile is not. **The EV impact is asymmetric:** short-put EV is *conservative* (25Δ put premium understated 13–41% vs a real smile — real markets price puts richer than ATM IV implies), while covered-call EV is *optimistic* (~6–12% overstated — real call IV typically sits below ATM). Apply a corresponding haircut to CC ranker `ev_dollars` until T0-1/T0-2 land. See `DECISIONS.md` D9 annotation and `docs/BRAIN_AUDIT_2026-06-11.md` §3.
...
| T0-6 | **Deep UST curve ≥2018** | ✅ DONE (R1/#338) | `treasury_yields.csv` now spans **1994→2026** (full tenor + sofr); the pre-2021 hardcoded ~5% fallback is latent (fires only pre-1994) and off the ranker path. | evaluate-input-correctness | BDH |
| T0-7 | **Corporate actions (wire the gate)** | WIRE (data ✅ on main) | `sp500_corporate_actions.csv` is **populated — 52,442 rows on `main`** (the "2-byte stub" claim is stale; no restore/re-pull). The split/M&A gate is still **inert because unwired** — see `WIRING_CAMPAIGN.md` Phase 3A. | remove-only-gate | have-already |
...
| T0-12 | **Short-interest / borrow** | WIRE (data ✅ on main) | Advisory sizing only — **no engine consumer yet** (`risk_manager.py` thresholds are hardcoded). Raw SI + DTC now on `main` (`data/bloomberg/broad_pull/short_interest/`, 134,035 rows); borrow rate + `pct_of_float` entitlement-blocked (manifest F). | advisory-sizing | have-already |
...
|---|---|---|---|---|---|
| ATM IV term structure (7d/14d/30d/60d/90d/180d/365d/730d) | `xxDAY_IMPVOL_100.0%MNY_DF` per tenor | `option_pricer`, `skew_dynamics`, `regime_detector` *(removed 2026-09-17)*, `forward_distribution` — true term structure for SVI/Nelson-Siegel; replaces static 30d/60d. Have 30d/60d already. | P1-high | BDH | evaluate-input-correctness |
...
| Realized vol (HV) at 10/20/30/60/90/120/180/260d | `nDAY_HV` family | `regime_detector` *(removed 2026-09-17)*, `tail_risk`, `forward_distribution` — IV−RV vol-risk-premium, regime bins, vol-cone backfill; 260d feeds the RV bootstrap. Have 30/60/90/260d. | P1-high | BDH | evaluate-input-correctness |
| IV Rank / IV Percentile | `IMPVOL_RANK` (BDP) or derive 252d rolling pct of ATM IV | `regime_detector` *(removed 2026-09-17)* — percentile-driven position sizing; advisor (Buffett value × vol). Theta `get_iv_rank` exists as cross-check. | P1-high | BDP | advisory-sizing |
| VIX family (VIX, VVIX, SKEW, VXN, RVX, VXEEM, OVX, GVZ) | `… Index` `PX_LAST` (BQL §5) | `regime_detector` *(removed 2026-09-17)*, `tail_risk`, `dealer_positioning` — SKEW tail regimes, VVIX vol-of-vol, sector/EM/oil/gold breadth. `sp500_vix_full.csv` holds 2015-01-02 → 2026-06-05 (17,274 rows) → optionally extend back to 2011. | P1-high | BDH | advisory-sizing |
| VIX futures term structure (UX1–UX7) | `UX1…UX7 Index` `PX_LAST` (BQL §4) | `regime_detector` *(removed 2026-09-17)* — contango/backwardation; UX1 vs UX3/UX6 spread. `vix_term_structure.csv` lacks the futures curve today. | P1-high | BDH | advisory-sizing |
...
| Vol-of-vol (VVIX or smile-curvature std) | `VVIX Index` `PX_LAST`; or `std(IV across moneyness)` | `regime_detector` *(removed 2026-09-17)*, `dealer_positioning` — VVIX>20 ⇒ unstable surface ⇒ size down GEX multiplier. | P2-medium | BDH | advisory-sizing |
| Historical vol cones (5/25/50/75/95 pct of RV) | Client-side rolling quantiles of the `nDAY_HV` series | `regime_detector` *(removed 2026-09-17)*, `forward_distribution` — regime-relative RV percentile; outlier flag vs cone. Store as `vol_cones.csv`. | P2-medium | manual-OMON | advisory-sizing |
| Single-name IV indices (AAPLV/TSLAV/NVDAV…, GVIX) | `…V Index` `PX_LAST` **(verify via FLDS)** | `regime_detector` *(removed 2026-09-17)*, `skew_dynamics` — high-confidence mega-cap regime signal; diagnostic, not load-bearing. | P3-nice-to-have | BDH | advisory-sizing |
...
| Beta (raw & adjusted; 1Y/2Y/5Y) | `BETA_RAW_OVERRIDABLE`, `BETA_ADJUSTED_OVERRIDABLE` (BDH) | `portfolio_copula` covariance, `portfolio_intelligence` *(removed 2026-09-17)*, risk-gate beta filter. Have snapshot only; need history. | P1-high | BDH | evaluate-input-correctness |
...
| Split/spin-off adjustment (PIT share count) | `SPLIT_ADJ_FACTOR`; `CORP_ACT_EFFECTIVE_DT` (BDS) / BQL §2 | `forward_distribution` jump-removal, sizing recalibration. `sp500_corporate_actions.csv` populated (52,442 rows on `main`). | P1-high | BDH | evaluate-input-correctness |
...
| VWAP history (EOD) | `VWAP` (BDH) | Liquidity gating, `signal_context` *(removed 2026-09-17)* timing, `cost_model` vs benchmark. Not critical for mid-market EV. | P2-medium | BDH | advisory-sizing |
...
| Cross-asset vol correlation (stock↔index↔VIX) | BQL VIX/VXN/SPX → local rolling corr; `CORRVOL`/`VOLSWAP` **(verify via FLDS)** | regime HMM panic/crowding flag, `portfolio_intelligence` *(removed 2026-09-17)*, `signal_context` *(removed 2026-09-17)*. | P2-medium | BQL | advisory-sizing |
...
| Microstructure noise ratio (HF vs LF vol) | None — compute from intraday bars | `signal_context` *(removed 2026-09-17)* entry gate, smile calibration noise. Needs minute bars. | P3-nice-to-have | manual-OMON | advisory-sizing |
...
| Short-interest ratio & days-to-cover | `eqy_short_interest`, `eqy_short_interest_pct_of_float` (BQL §3) | `risk_manager` crowded-short, R10 single-name squeeze soft-warn (>25% float + >5% NAV). Now on `main` (`broad_pull/short_interest/`, 134,035 rows; raw SI + DTC — `pct_of_float` entitlement-blocked). | P1-high | BQL | advisory-sizing |
...
| Corporate actions (splits/special divs/M&A) | `cac_announcement_date`, `cac_type`, `cac_ratio` (BQL §2) | `event_gate` early-assignment lockout, `binomial_tree` assignment fee, deal-arb. **File populated (52,442 rows on `main`) — wire the gate (Phase 3A), no refill.** | P1-high | BQL | remove-only-gate |
...
| Dataset | Bloomberg vehicle | Engine use | Benefit | Feasibility | §2 role |
|---|---|---|---|---|---|
| UST curve history (par/zero 1m–30y) | `BDH USGG{tenor} Index PX_LAST` 2015–2026 | Risk-free input to BSM; term-structure discounting, HAR-RV. `treasury_yields.csv` now spans **1994→2026** (R1/#338) — the pre-2021 hardcoded-5% gap is closed. | ✅ DONE | BDH | evaluate-input-correctness |
...
| Volatility indices (SKEW/VVIX/VXN/MOVE/OVX/GVZ/RVX) | `BDH … Index PX_LAST` (partly BQL §5) | `regime_detector` *(removed 2026-09-17)* tail/vol-of-vol/sector regimes (SKEW<100 crash signal). Partly in `vol_indices.csv`. | P1-high | BDH | advisory-sizing |
| IV term structure & calendar dynamics (30/60/90/120d ATM) | `BDH 30/60/90/120DAY_IMPVOL_100.0%MNY_DF` | `regime_detector` *(removed 2026-09-17)*/`monte_carlo` contango vs backwardation. Have 30/60d (EXTRACTION_GUIDE §5). | P1-high | BDH | evaluate-input-correctness |
| Vol-of-vol (VVIX + realized) | `VVIX Index` (BQL §5); realized from VIX returns | `regime_detector` *(removed 2026-09-17)* vol-of-vol classification; VVIX>30 = vega convexity. | P1-high | BDH | evaluate-input-correctness |
| SKEW tail-risk index | `SKEW Index` (BQL §5) | `regime_detector` *(removed 2026-09-17)* tail regime + Taleb; cross-check `tail_risk` POT-GPD. Already on disk. | P1-high | BDH | evaluate-input-correctness |
| Corporate actions (splits/special divs/M&A) per SPX | `BQL cac_*` (§2) | `event_gate` lockout; `sp500_corporate_actions.csv` populated (52,442 rows on `main`) — wire the gate. | P1-high | BQL | remove-only-gate |
| Real yields & TIPS / inflation swaps | `USGG5REAL Index`, `USSW2/5/10 Index` (BDH) | `regime_detector` *(removed 2026-09-17)* inflation-expectation regime; portfolio drift/CVaR vs real-yield shocks. | P2-medium | BDH | advisory-sizing |
...
| Financial conditions (NFCI/MOVE/CVIX) | `BDH NFCI/MOVE/CVIX Index` | `regime_detector` *(removed 2026-09-17)* tightening/easing; rates-vol degrades long-dated options. | P2-medium | BDH | advisory-sizing |
...
| Dataset | Bloomberg vehicle | Engine use | Benefit | Feasibility | §2 role |
|---|---|---|---|---|---|
| Corporate actions: stock splits (announce/effective) | `CAC_ANNOUNCEMENT_DATE`, `CAC_EFFECTIVE_DATE`, `CAC_TYPE`, `CAC_RATIO` (BQL §2) | `event_gate` 3-day split lockout (early assignment on short calls). **File populated (52,442 rows on `main`) — wire (Phase 3A).** | P1-high | BQL | remove-only-gate |
...
| IV term structure & calendar spread (90/120d add) | `30/60/90/120DAY_IMPVOL_100.0%MNY_DF` | `regime_detector` *(removed 2026-09-17)*/`monte_carlo` contango/backwardation; strangle timing. On disk via EXTRACTION_GUIDE §5. | P1-high | BDH | evaluate-input-correctness |
| Vol-of-vol (VVIX + realized) | `VVIX Index` (BQL §5); compute realized from VIX returns | `regime_detector` *(removed 2026-09-17)* vol-of-vol; VVIX>30 vega convexity → strangle vol-buy. | P1-high | BDH | evaluate-input-correctness |
| SKEW index | `SKEW Index` (BQL §5) | `regime_detector` *(removed 2026-09-17)* tail regime + Taleb; SKEW>120 hedging, <100 complacency. Feeds `tail_risk`/`monte_carlo`. | P1-high | BDH | evaluate-input-correctness |
...
| Macro surprise index (Citi/Bloomberg) | `CESIUSD Index`, `BESIUSD Index` | `regime_detector` *(removed 2026-09-17)* macro gauge (>0 better-than-expected); IV term-structure expectation. Clean daily signal. | P2-medium | BDH | evaluate-input-correctness |
...
| W-5 | **Wire corporate-actions into the event gate** | `engine/event_gate.py` | Split/special-div/M&A gate is inert (**unwired**). The file is **already populated on `main` — 52,442 rows** (no restore/re-pull; the "2-byte stub" claim is stale); only the `event_gate` wiring remains (`WIRING_CAMPAIGN.md` Phase 3A). | remove-only-gate |
...
|---|---|---|---|---|
| ~~Treasury / UST curve~~ **✅ DONE** | — (R1/#338) | `treasury_yields.csv` | **Already backfilled 1994→2026** — the pre-2021 hardcoded-5% gap is closed. | evaluate-input-correctness |
...
| Corporate actions **✅ on main — WIRE** | **already populated (52,442 rows)** — don't pull | `sp500_corporate_actions.csv` | Split/M&A gate is inert (data present; **unwired** — Phase 3A). | remove-only-gate |
| Short interest & borrow **✅ on main — WIRE** | **already pulled** (`broad_pull/short_interest/`, 134,035 rows) | `sp500_short_interest.csv` | Crowded-short sizing input; wire it. Borrow + `pct_of_float` entitlement-blocked (raw SI + DTC only). | advisory-sizing |
...
| SKEW / vol-of-vol family | §5 `BQL px_last SKEW/VVIX/VXN/RVX/OVX/GVZ…` | `vol_indices.csv` | Tail + vol-of-vol regime; `sp500_vix_full.csv` holds 2015→2026 (17,274 rows). | advisory-sizing |
