...
- **No per-strike option chains.** `OMON` is a manual, screen-only export — there is no `BDS`/`BQL` mass-pull of the full chain. Per-strike open interest, per-strike greeks, and the OI-weighted gamma profile that `dealer_positioning.py` needs **must come from Theta** (we already hold ~185M EOD-chain rows 2016–2026). Anything in the tables below tagged `use-theta` / `have-already` reflects this.
...
| VIX family (VIX, VVIX, SKEW, VXN, RVX, VXEEM, OVX, GVZ) | `… Index` `PX_LAST` (BQL §5) | `regime_detector`, `tail_risk`, `dealer_positioning` — SKEW tail regimes, VVIX vol-of-vol, sector/EM/oil/gold breadth. `sp500_vix_full.csv` near-empty → backfill 2011–2026. | P1-high | BDH | advisory-sizing |
...
| Liquidity / per-strike OI / bid-ask / greeks | Theta `/v3/option/snapshot/quote` (Bloomberg has no per-strike) | `wheel_runner`, `option_pricer`, `dealer_positioning` — strike liquidity, slippage, OI-weighted gamma/pinning. We hold ~185M Theta rows. | P1-high | use-theta | evaluate-input-correctness |
...
| Per-strike OI & IV surface | OMON (manual) or **Theta EOD chains 2005–2026 (have)** | `dealer_positioning` GEX/walls, `skew_dynamics` by-delta, `option_pricer` OI-weighted smile. | P1-high | have-already | evaluate-input-correctness |
...
| Short-interest ratio & days-to-cover | `eqy_short_interest`, `eqy_short_interest_pct_of_float` (BQL §3) | `risk_manager` crowded-short, R10 single-name squeeze soft-warn (>25% float + >5% NAV). `sp500_short_interest.csv` stale (2 bytes). | P1-high | BQL | advisory-sizing |
...
| W-3 | **Wire Theta per-strike OI into `DealerPositioningAnalyzer`** | `engine/dealer_positioning.py` | Dealer multiplier currently inert at 1.0 → real OI-weighted GEX from the ~185M Theta rows we already hold. | advisory-sizing |
...
| Short interest & borrow | §3 `BQL eqy_short_interest…` | `sp500_short_interest.csv` | Crowded-short sizing input (stale 2-byte file today). | advisory-sizing |
...
| SKEW / vol-of-vol family | §5 `BQL px_last SKEW/VVIX/VXN/RVX/OVX/GVZ…` | `vol_indices.csv` | Tail + vol-of-vol regime; `sp500_vix_full.csv` near-empty. | advisory-sizing |
