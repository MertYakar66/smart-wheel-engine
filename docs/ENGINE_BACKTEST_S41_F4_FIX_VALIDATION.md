...
- **For `docs/PRODUCTION_READINESS.md`:** §3 Blocker B1 marked "partially closed" by PR #260 is the right framing. S41 confirms the partial closure: the engine *is* meaningfully more cautious in vol-cluster regimes; the named cases remain unclosed (and R10's per-name cap is what bounds them).
