...
| I11 | R11 risk-budget parameter study **(SPEC + RESULTS)** | NOT a null result: a VIX-*level* > 25 top-bin size-down is favorably asymmetric in every well-powered crisis fold (2020 +$86k averting the −$1,305/ctr tail; 2022 +$3.5k — θ≥27.5 fails the 2022 fold). **Conditional R11 (VIX>25) supported.** VIX-level succeeds where I10's `rv_ratio` failed. §6 residual gap: VIX-level misses idiosyncratic calm-market single-name tails (complemented by the armed R10, #303). | `…_I11_RISK_BUDGET_STUDY.md` (+ `…_SPEC.md`) |
