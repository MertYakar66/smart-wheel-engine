...
| I11 | R11 risk-budget parameter study **(SPEC — observe-only, execution pending)** | Designs the study that picks R11's VIX-*level* size-down threshold by leave-one-crisis-out cost/benefit (2020-onset bleed averted vs 2022/recovery premium forgone; 2022 is a false-positive cost, not a miss — VIX level, unlike I10's `rv_ratio`, fires there). Coarse threshold; null-result → unconditional top-bin haircut | `HEAVY_VERIFY_2026-05-31_I11_RISK_BUDGET_STUDY_SPEC.md` |
