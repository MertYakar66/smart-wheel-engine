...
|---|---|---|---|
| **B1** | **F4 tail-risk widening fix.** **(Mostly shipped — 2026-05-27.)** Fix B1 (regime-conditioned widening) on `claude/fix-f4-regime-conditioned-widening` closes UNH 2024-11 (ev$ +$114 → −$118, cvar_5 past −10% of collateral). Fix C (worst-of-two evaluation) preserves COST 2022-04 ev$ flip (+$62 → −$25) without regressing into the +EV the naïve sample-source switch produced (see `docs/F4_TAIL_RISK_DIAGNOSTIC.md` §11). COST prob_profit remains 0.833 — out-of-scope follow-up requires a name-level regime signal (ticker-specific RV/baseline) that fires when the HMM under-detects idiosyncratic single-name drawdowns. S22/S27 backtest re-run on Fix-B1+C also remains open. | `tests/test_f4_tail_risk_regression.py` + `tests/test_f4_tail_risk_gap.py` (UNH gap assertion flipped). | targeted unit tests pass; full S22 / S27 backtest re-run shows 2022 mean realized > $0. |
