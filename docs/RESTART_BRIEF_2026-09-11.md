...
The wheel is a two-step options income loop on stocks you would be willing to own. Step one: sell a cash-secured put. A put obliges you to buy 100 shares at a fixed price (the strike) by a fixed date (expiry). You are paid cash now (the premium) and you park the full purchase price in cash, so a fall in the stock can never trigger a margin call; you can still be assigned the shares and lose money on them. If the stock stays above the strike, you keep the premium. If it falls below, you are assigned the shares. Step two: sell a covered call on those shares, the mirror promise to sell them at a higher strike, again for premium, until they are called away. Then start over.

The engine ranks these trades across the S&P 500. For each name it estimates the expected value (EV) of selling one put: the probability-weighted average dollar outcome, net of costs, built from that stock's own price history and its implied volatility (IV, the size of move the options market is pricing in). It also emits `prob_profit`, its estimate of the chance the trade ends profitable. It answers one question: which put (or covered call, or strangle, a put and a call sold together on the same stock) to sell, and in what order.

What it does not do. It never places, modifies, or cancels an order; there is no order-routing surface anywhere in the repository and every brokerage path is read-only by invariant (read). It does not evaluate or score a position once open; the exit evaluator (DECISIONS D25) is proposed, not adopted, and the position tracker can roll a position without any gate (held finding F1 below). It does not forecast dollars: the docs record `ev_dollars` as a risk-aware ranking score, not a dollar-profit forecast. It does not see a crisis before the market has repriced. It does not beat the index.
...
2. **Volatility and strike.** At-the-money IV is read as of the date, with dividend yield and the risk-free rate. Black-Scholes is solved backward for the strike at delta 0.25 over a 35-day horizon, rounded to 50 cents, and a synthetic premium is priced with the same formula. A real end-of-day market mid (the midpoint of bid and ask) replaces it when the option-premium "rail" (a locally produced quote feed from Theta) has a matching quote; without those gitignored files every premium is synthetic and `edge_vs_fair` is zero by construction, so the engine cannot tell whether an option is rich or cheap. The synthetic premium has a known bias: Bloomberg IV carries no skew, so it understates 25-delta put premium by 13 to 41% (short-put EV conservative) and overstates call premium by 6 to 12% (covered-call EV optimistic) (DECISIONS D9, reported). Known hole (held finding F4): when PIT IV is missing at a historical date, the ranker falls through to the dateless 2026 snapshot IV, a look-ahead.
...
4. **Scenarios.** The last five years of closes are cut into non-overlapping 35-day chunks, roughly 35 historical outcomes. Caveat D21: 35 calendar days are indexed as 35 trading bars, about 46% too long; fixing that alone makes the top-bin over-confidence worse (a representative `prob_profit` moves from 0.833 to 0.886), which is why D21 is bundled with a recalibration (reported). When 30-day realized volatility is at least 1.30 times the one-year baseline, the spread is widened by up to 15% (the May "F4" fix).
...
Reported, not verified: there are three verdict ladders, not one. The dashboard's `/api/candidates` does not run the reviewer and uses its own (proceed if EV of at least 10 and `prob_profit` of at least 0.65), and the TradingView webhook path has a third that adds a chart-agreement check and an earnings-within-five-days skip. All three share the $10 proceed threshold and diverge on overlays by design.
...
| **proven** | Full-menu ordering by `ev_dollars` is worthless; `prob_profit` ordering stays positive by regime | `ev_dollars`: pooled rank correlation +0.02, confidence interval [−0.02, +0.06], quiet-bull regimes −0.11; `prob_profit`: +0.15 to +0.18 in every regime (read) |
| **proven** | The data path is PIT-clean and deterministic, and the invariant holds | truncating post-date rows gave zero diffs; the freeze fixture reproduces to 1e-7; zero executed rows with EV at or below zero across ten configurations (read). Standing PIT limitations survive these tests: F4, the dateless sector and IV snapshot (#354), and the live FRED credit fetch |
| **proven** | A mechanical $500k book is usually profitable | 9 of 11 windows positive at full friction, mean +15.4% per 18 months, mean max drawdown 16.4%; an index comparison of drawdowns exists only for two earlier windows (read). The $200k companion: 7 of 8 years positive, mean +14.3%, but one window drew down 34% in its second month (reported) |
...
| **proven defect** | Top-bin `prob_profit` over-confidence, everywhere | PROJECT_STATE: the top bin is over-optimistic by 10 to 18 points across all ten configurations, and 9 of 10 exceed 10 points; the May calibration study: (0.95, 1] forecast 0.966 versus realized 0.695 on 105 trades; the $500k campaign: [0.9, 1.0] 0.922 versus 0.854 on 14,558 rows; your own 28 real top-bin puts: 0.936 predicted versus 0.821 realized (read). The gap's size depends on the convention used. The full real-account record (456 legs, March 2025 to June 2026) shows puts slightly conservative overall but over-confident in the top bin, and covered calls under-confident (reported). The $500k bin figures are not yet reproducible from committed scripts |
| **measured, partial** | Survivorship inflates the edge | adding three failed banks to a 100-name 2022 to 2024 run cut the profit from +$263,695 to +$105,696, so 59.9% of that driver's own profit was illusory; per-candidate-day edge fell 23%; the EV model's mean did not move (read). This is a floor from three added names on a 100-name base; the 500-name point-in-time run has not been done |
| **proven defect** | Miscalibration concentrates at regime transitions the HMM has not caught | rows the HMM labels bear are nearly calibrated at the top (−3.1 points) while rows labelled normal or bull-quiet inside a deteriorating market miss by −9.4 to −10.9 points; no simple point-in-time signal detects the transition, which is why R11 uses the external VIX rather than the HMM (read) |
...
| **refuted** | Crisis detection | 89% of names flagged positive-EV at the March 2020 entry, realizing −$1,305 per contract at 82% assignment; the 2007 to 2009 lockbox: refusal failed every month and the entry rate rose 3.15 times into Lehman week; survival came from assignment-and-hold (read). The 2008 result is premium-coupled (synthetic premiums at 2008 IV inflate the credit side) and real option marks before 2016 do not exist, so that caveat may never close |
...
| **open** | True out-of-sample | the holdout is leakage-certified and the parameter freeze exists, but static constants saw all history; refitting the regime overlay collapses out of sample; no forward record of engine outputs captured at decision time exists; the real-account calibration is a retrospective replay of real trades (read) |
| **open** | Capacity and sizing | at the S34 configuration a $1M book is capital-bound at every contract size; the tracker is contract-blind and every locked study uses one contract, so multi-contract sizing has never been validated (reported) |
| **open** | Live behaviours never modelled | hold-to-expiry only; rolls (one study found a credit roll beats holding by about $195 per contract, yet the roll path bypasses the caps); early assignment; multi-contract sizing (reported) |
| **open** | Premium realism | with real Theta mids, book economics moved by double digits in both directions (W05 +4.8% to +2.1%, W09 +15.0% to +21.5%) while calibration did not change; absolute EV magnitudes are not economically meaningful until a full-universe premium rail exists (read) |
| **open** | Reproducibility of the $500k report | the rank-correlation figures and the window table reproduce from committed artifacts; the Brier score, the calibration bin table, and the transition finding do not, because the deriving scripts were not committed (read); beyond that campaign only the four S27, S32, S34, S35 snapshots are pinned by a committed harness, and the S38, S40, S44, calibration, $200k, and MAG7 studies ran from uncommitted throwaway harnesses (reported) |

Verifier notes worth keeping: the current committed regression snapshots carry rank correlations S27 0.178, S32 0.176, S34 0.313, S35 0.501, and older doc tables that quote other values are stale. The $200k companion campaign also ran with R10 armed, so the $500k campaign is not the only armed run; that companion found a look-ahead inside `_compute_live_nav` for any historical simulation. NFLX is not corrupted (uniform back-adjustment); only #454 remains a live scale defect (read). Read every engine-versus-market number with its benchmark attached: S38 uses SPY with dividends, S40 and S44 use equal weight price-only, and the $500k campaign uses the S&P price return without dividends (read).
...
**What shipped in July while you were away** (read): 07-02 to 07-04, rail date coherence, the earnings-lockout restore, and the frontier bump to 2026-07-02 with a 14-branch merge-down; 07-08, the D28 repository audit, the paper book, the Monte Carlo simulator, and the parameter-freeze gate; 07-15, all ten audit fixes; 07-17 and 07-18, the dashboard Trades tab and time-weighted returns; 07-21, the hardening wave, the data-sufficiency review, the V1 to V6 validation harnesses, and the held-findings tests; 07-22, the $500k campaign run in one day; 07-28, its report and corrections, then the operating-model consolidation.
...
| F4 IV-fallback guard | design written | trio consent; moves backtest baselines | both | historical runs only; live ranking unchanged |
...
| Remaining audit HIGH items (the stress test substitutes strike for a missing spot; a dead slippage parameter; a rotated price column in `data/consolidated_loader.py`) | open, not scheduled | none | agent, then Operator | no (off the EV path) |

Work that can start today without any decision from you: the Dashboard-branch adapter PR, the docs currency pass, the worklog status truth-pass, the post-#523 follow-ups, and the #454 data fix up to the re-pin (read).
...
9. **R11 threshold.** Keep VIX 25 (D23, and the IBKR guardrail "do not widen"), move to about 27.5 (the July validation's lift crossover), or accept that its whole-book effect is statistically zero. On your own 28 top-bin puts R11 would have fired twice and caught none of the five failures (reported). No recommendation.
...
11. **Calls the readers raised that no document has asked you yet.** Reviewer reach on the default path (keep the chart-first stop at R2, make it a note so R5 to R11 run on engine evidence, or route the dashboard's candidates through the reviewer; on your rig, if no screenshots directory or chart feed exists, every cockpit dossier stops at R2 today). Chain data on the Bloomberg path (accept dealer positioning and R6 as Theta-only, add a chain accessor, or strip the placeholder columns). The delta and Kelly caps (recalibrate and arm, or leave R9 and R10 as the only concentration gates). The macro gate (fund the entry-proximity version or retire it). Which ranking signal any supervised use follows (`ev_dollars`, `prob_profit`, or a blend). The 500-name survivorship run, or a standing haircut on every survivor-universe number. The top-bin haircut for supervised use (10 to 15 points suggested). When to flip deep history on. TradingView tunnel versus polling.
...
- **D16 token, D17 caps:** a receipt proving a trade came from the ranker with positive EV, which the tracker must present before opening a position when strict mode is on; and the sector and single-name hard blocks.
- **NAV:** net liquidation value, the account's total value; the caps are percentages of it.
- **VIX:** the market's expected-volatility index, the basis of R11.
- **Option chain, dealer positioning:** the list of all listed options on a stock, and the hedging position of market makers inferred from it (short gamma, put wall); both need Theta data.
- **Mid, rail, larder:** the midpoint of bid and ask; the locally produced real-quote feed; the locally stored Theta option-history corpus.
- **At-the-money, top bin, haircut:** a strike near the current price; the highest `prob_profit` bucket (0.9 to 1.0); a deliberate discount applied to a forecast.
- **Roll:** close the current short option and open a new one at a later expiry or a different strike.
- **Holdout, frozen, executed slice, pooled:** data the model never trained on; parameters fixed at a past date; only the trades a book actually took; every ranked row together.
- **Equal weight:** a benchmark holding every stock in the universe in equal dollar amounts.
- **S27, S32, S34, S35:** the four backtest configurations whose results are locked as regression snapshots.
- **M2, M4:** two brain-audit items in the re-baseline bundle: apply the realized-vol widening to the covered-call, strangle, and roll paths; wire Almgren-Chriss size impact into the cost model.
