...
### S28 — Skew data absence on Bloomberg connector

**Purpose.** Realism counterpart to S27, but on the *negative-result*
...
diagnostic columns of `rank_candidates_by_ev`. S28 asks: is any of
...
- **(F5 — observability, the trader-facing surface) The
  ...
  (`drops` missing on `suggest_rolls`) and S27 F2
...
- **Sanity follow-up Sn:** once Fix #1 (observability) is in,
  re-run S28 on a Theta replay (queued S6) to verify
...
- **Dealer positioning (S30 candidate, deferred this
...
- **Single connector (Bloomberg).** The S28 verdict ("skew
