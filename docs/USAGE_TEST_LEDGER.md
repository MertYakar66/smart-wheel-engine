...
### S29 — HMM regime-multiplier realism (April 2025 vol spike)
...
normal: 1.0, bull_quiet: 1.25`). S29 tests both downward and
...
- **(F5 — observability gap) No cross-asset coherence
  ...
  S27 F2, S28 F5 — the campaign's observability theme).
...
- **The HMM is the realism counter-example to S28's skew
...
- **HMM-cross-asset Sn:** the natural follow-on to S29 is a
  ...
  per-ticker convergence in S29 was a small-sample artifact
  and the F5 macro-aggregator design would need a smarter
  threshold.

- **Dealer positioning (S30 candidate, deferred this cycle):**
...
- **Cache not exercised.** The HMM cache (keyed by
  ...
  re-fit. The 2-as_of test in S29 always re-fits (the tail
