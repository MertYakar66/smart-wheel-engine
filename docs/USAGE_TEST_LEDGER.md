...
### S43 — S38 re-run on post-F4 engine (PR #260 dollar-improvement test)
...
NAV −12.1%, executed −22%). S43 tests the same question at the
...
| Aspect | Engine (S43) | External reference / prior Sn | Verdict |
|---|---|---|---|
| Section-2 invariant preservation | 0 breaches on PUTs both pre/post | F4 fix is downgrade-only by design | ✓ |
| Cross-config consistency with S41 | S43 ρ −1.0% vs S41 ρ −3.3%; S43 NAV +0.4% vs S41 NAV −12.1% | Universe size + window length dilute per-trade F4 impact | ✓ Different magnitude same direction-of-effect on ρ |
...
| Deployment-matrix amendment (PR #263) | S43 result −58.45pp Univ-EW; PR #263 cites −52pp SPY | Within error bars; matrix verdict unchanged | ✓ No re-revision needed |
| F4 + R10 deployment bundle (S41 framing) | S43 reinforces: PR #260 alone not value-creating at \$1M/100t | F4 = frequency guard; R10 = magnitude guard; bundle closes B1 | ✓ Consistent |
...
- **PROD_READINESS deployment matrix (PR #263 amendment) requires
  no revision based on S43.** The −52pp framing is within the
  observed range.
...
- **For the F4 docs-sync follow-up flagged in PR #263's
  Unresolved:** S43 reinforces that PROD_READINESS §1/§3/§6 should
  cite PR #260 as "shipped, but value-creating only in bundle with
  R10" rather than open-vs-shipped status binary. S41 + S43
  together are the validation that B1 closure requires both PRs.
- **For PR #263 reviewers:** S43 confirms the deployment matrix
...
Full doc: `docs/ENGINE_BACKTEST_S43_S38_POSTF4_RERUN.md`.
