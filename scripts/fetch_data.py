...
def drive_service():
    try:
        ...
        sys.exit(
            "pip install google-api-python-client google-auth  (needed for download; --check does not)"
        )
...
def main() -> int:
    ...
    ap.add_argument(
        "--check", action="store_true", help="verify local files against the manifest; no Drive"
    )
    ap.add_argument(
        "--served-only", action="store_true", help="only the 10 _FILES the live engine reads"
    )
    ap.add_argument(
        "--include-deep", action="store_true", help="also the opt-in deep-history archive"
    )
    args = ap.parse_args()
    ...
    print(
        f"{'CHECK' if args.check else 'FETCH'}: {len(files)} file(s) "
