...
def _reshape(raw, fmap, suffix: str):
    ...
    w = raw.pivot_table(
        index="date", columns="field", values="value", aggfunc="first"
    ).reset_index()
    w.columns.name = None
...
def pull_ohlcv(tk, ws, we):
    w = _reshape(
        blp.bdh(tickers=tk + " Equity", flds=OHLCV_FLDS, start_date=ws, end_date=we), OHLCV_MAP, ""
    )
    if w is None:
        return None
...
def pull_voliv(tk, ws, we):
    w = _reshape(
        blp.bdh(tickers=tk + " Equity", flds=VOLIV_FLDS, start_date=ws, end_date=we, Fill="P"),
        VOLIV_MAP,
        "",
    )
    if w is None:
...
def pull_liq(tk, ws, we):
    w = _reshape(
        blp.bdh(tickers=tk + " Equity", flds=LIQ_FLDS, start_date=ws, end_date=we, Fill="P"),
        LIQ_MAP,
        "",
    )
    if w is None:
...
def main():
    ...
    def flush():
        for key, acc in (
            ...
            if key in ("ohlcv", "voliv", "liq") and len(df):
                df = df.drop_duplicates(subset=["date", "ticker"], keep="last").sort_values(
                    ["ticker", "date"]
                )
    ...
    print(
        f"Processing {len(uni)} delisted names (offset={offset} limit={limit or 'all'})", flush=True
    )
            ...
            if oh is not None and len(oh):
                ...
                if rate > VALVE:
                    ...
                    if len(bad):
                        ...
                        b["window"] = f"{ws}..{we}"
                        dropped.append(b)
                    oh_all.append(cleaned)
                    st["ohlcv_rows"] = len(cleaned)
                    st["dropped"] = len(bad)
    ...
    print(
        f"  OHLCV resolved : {(sdf['ohlcv_rows'] > 0).sum()} names, {len(oh_df):,} rows  "
