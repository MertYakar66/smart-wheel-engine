...
def pull_snapshot(as_of: str) -> pd.DataFrame | None:
    r = to_native(
        blp.bds("SPX Index", "INDX_MWEIGHT_HIST", END_DATE_OVERRIDE=as_of.replace("-", ""))
    )
...
def main():
    ...
    agg = (
        mem.groupby("member_ticker_and_exchange_code")["as_of_date"]
        .agg(first_in_index="min", last_in_index="max", n_snapshots="count")
        .reset_index()
    )
    agg = agg.rename(columns={"member_ticker_and_exchange_code": "ticker"})
    missing = agg[~agg["ticker"].isin(have)].copy()
    print(
        f"DELISTED/MISSING: {len(missing)} of {distinct} distinct names have NO OHLCV "
    ...
    print(
        f"WROTE {os.path.normpath(DELISTED)} ({len(missing)} delisted/missing names; "
