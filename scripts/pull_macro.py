...
def pull_instrument(inst: str, ticker: str, floor: str, end: str) -> pd.DataFrame | None:
    ...
    wide = wide.rename(
        columns={k.lower() if k.lower() in wide.columns else k: v for k, v in FIELD_MAP.items()}
    )
...
def main():
    ...
    if os.path.exists(OUT) and os.path.getsize(OUT) > 100:
        ...
        print(
            f"Existing sp500_macro.csv: {len(existing):,} rows, "
    ...
    if smoke:
        print("\n--- spx head ---")
        print(fresh_all.head(3).to_string(index=False))
        print("\n--- spx tail ---")
        print(fresh_all.tail(3).to_string(index=False))
        return
