...
FIELD_SETS = [
    {
        "names": [
            "30D_IMPVOL_100.0%MNY_DF",
            "60D_IMPVOL_100.0%MNY_DF",
            "90D_IMPVOL_100.0%MNY_DF",
...
for fs in FIELD_SETS:
    try:
        test = blp.bdh(
            tickers=[tickers[0]], flds=fs["names"], start_date="2025-01-01", end_date="2026-03-20"
        )
        if len(test) > 0:
            working_fields = fs
...
if working_fields is None:
    ...
    for i, t in enumerate(tickers):
        try:
            data = blp.bdp(
                t, ["IVOL_30D", "IVOL_60D", "IVOL_90D", "HIST_PUT_IMP_VOL", "HIST_CALL_IMP_VOL"]
            )
            if len(data) > 0:
    ...
    out_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "bloomberg", "sp500_options_greeks.csv"
    )
