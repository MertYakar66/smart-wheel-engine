...
def pull_one(name, cfg, tickers, smoke):
    ...
    wide = wide.rename(
        columns={c: bbg_to_col.get(c.upper(), c) for c in wide.columns if c != "ticker"}
    )
    ...
    order = (
        (["ticker"] + list(cfg["fields"]))
        if cfg["ticker_first"]
        else (list(cfg["fields"]) + ["ticker"])
    )
...
def main():
    ...
    for name, cfg in SNAPSHOTS.items():
        ...
        if empty:
            print(
                f"  !! WARNING fields entirely NULL (possible invalid field nulling batch): {empty}"
            )
        if smoke:
