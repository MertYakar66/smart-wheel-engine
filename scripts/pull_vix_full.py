...
def main():
    ...
    if os.path.exists(OUT) and os.path.getsize(OUT) > 50:
        ...
        print(
            f"existing: {len(existing):,} rows, {existing['date'].min()} -> {existing['date'].max()}"
        )
    ...
    if smoke:
        print(fresh.groupby("instrument")["date"].agg(["min", "max", "count"]).to_string())
        return
