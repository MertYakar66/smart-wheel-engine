...
Locked in to prevent any custom provider (an MCP-backed one was the
original worry) from bypassing EnginePhaseReviewer's rules. See CLAUDE.md §2 and §7.
"""
...
@dataclass
class _FakeChartProvider:
    """Stand-in for a custom ChartContextProvider.
...
def test_custom_provider_phase_contradiction_routes_through_reviewer():
    ...
    provider = _FakeChartProvider(chart_context=_chart(phase="compression", spot=100.0))
...
def test_custom_provider_pristine_chart_cannot_upgrade_negative_ev():
    ...
    provider = _FakeChartProvider(chart_context=_chart(phase="post_expansion", spot=100.0))
    dossiers = build_dossiers(ev_frame=ev_df, provider=provider, top_n=5)
    assert len(dossiers) == 1
    d = dossiers[0]
